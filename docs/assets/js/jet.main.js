...
(function () {
    ...
    function _initSingleLoad (_this, item, i, list, call) {
        ...
        if (Jet.load.__loadStore[src]) {
            ...
            _JT.load(_addAssetsPath(src), function (html) {
    ...
    function _getSrc (v, t, item) {
        ...
        if (item && item._JT_attr(_jpath) === 'false') {
            if (_canUse('router')) {
                src = Jet.router.conf.base + _dealSrc(v);
            } else {
                src = _checkTailFix(v, t);
            }
        } else if (_canUse('res')) {
            src = Jet.res.getSrc(v, t);
        } else if (_canUse('router')) {
            src = Jet.router.conf[t] + _dealSrc(v);
        } else {
            src = _checkTailFix(v, t);
        }
        return _addAssetsPath(src);
    }
    ...
    function _loadSingleLib (name, index) {
        ...
        if (_libList.indexOf(name) !== -1) {
            src = _addAssetsPath('assets/js/jet-lib/' + name + '.js');
        } else if (_defineLibs[name]) {
            src = _addAssetsPath(_defineLibs[name]);
        }
